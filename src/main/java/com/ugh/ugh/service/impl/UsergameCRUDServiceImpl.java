...
public class UsergameCRUDServiceImpl implements IUsergameCRUDService {
	...
	@Autowired
	private IUserGameRepo userGameRepo;
	
	@Override
	public void create(GameStatus gameStatus, User user, Game game) throws Exception {
		if (gameStatus == null || user == null || game == null) throw new Exception("One or more input fields are incorrect");
		if (userGameRepo.existsByUserAndGame(user, game)) throw new Exception("This game already exists in user's library");
	...
	public ArrayList<UserGame> retrieveAll() throws Exception {
		if (userGameRepo.count() == 0) throw new Exception("There are no UserGames to retrieve");
	...
	public UserGame retrieveById(long id) throws Exception {
		...
		if (!userGameRepo.existsById(id)) throw new Exception("No such UserGame exists with the id: " + id);
		UserGame userGameFromDB = userGameRepo.findById(id).get();
		return userGameFromDB;
	}
	
	@Override
	public void updateById(long id, GameStatus gameStatus, User user, Game game) throws Exception {
		UserGame userGameFromDB = retrieveById(id);
		if (gameStatus == null || user == null || game == null) throw new Exception("One or more input fields are incorrect");
		
		if (!userGameFromDB.getGameStatus().equals(gameStatus)) userGameFromDB.setGameStatus(gameStatus);
		if (!userGameFromDB.getUser().equals(user)) userGameFromDB.setUser(user);
		if (!userGameFromDB.getGame().equals(game)) userGameFromDB.setGame(game);
