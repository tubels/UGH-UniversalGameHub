...
public class GameCRUDServiceImpl implements IGameCRUDService {
	...
	GameCRUDServiceImpl(IGameRepo gameRepo, IDeveloperRepo devRepo, IPublisherRepo publRepo, IGenreRepo genreRepo) {
	...
	public ArrayList<Game> retrieveAll() throws Exception {
		if (gameRepo.count() == 0)
			throw new Exception("There are no games to retrieve");
	...
	public Game retrieveById(long id) throws Exception {
		...
		if (!gameRepo.existsById(id))
			throw new Exception("There are no games with the id: " + id);
	...
	public void updateById(long id, String title, float price, String description, LocalDate releaseDate,
		...
		if (gameFromDB.getPrice() != price)
			gameFromDB.setPrice(price);
		if (!gameFromDB.getDescription().equals(description))
			gameFromDB.setDescription(description);
		if (!gameFromDB.getReleaseDate().equals(releaseDate))
			gameFromDB.setReleaseDate(releaseDate);
		if ((gameFromDB.getPublisher() == null) || !gameFromDB.getPublisher().equals(publisher))
			gameFromDB.setPublisher(publisher);
		if ((gameFromDB.getDeveloper() == null) || !gameFromDB.getDeveloper().equals(developer))
			gameFromDB.setDeveloper(developer);
		if ((gameFromDB.getGenres() == null) || !gameFromDB.getGenres().equals(genreCollection))
			gameFromDB.setGenres(genreCollection);
