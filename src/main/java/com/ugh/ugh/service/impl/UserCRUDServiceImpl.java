...
public class UserCRUDServiceImpl implements IUserCRUDService, UserDetailsService {
	...
	public UserCRUDServiceImpl(IUserRepo IuserRepo, IUserRepo userRepo, PasswordEncoder passwordEncoder) {
	...
	public ArrayList<User> retrieveAll() throws Exception {
		if (userRepo.count() == 0)
			throw new Exception("There are no Users to retrieve");
	...
	public User retrieveById(long id) throws Exception {
		...
		if (!userRepo.existsById(id))
			throw new Exception("There are no Users with the id: " + id);
